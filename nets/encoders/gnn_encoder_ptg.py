...
class GNNLayer(tg.nn.MessagePassing):
    ...
    def forward(self, h, edge_attr, graph):
        """
        Args:
            h: Input node features (B x V x H)
            edge_attr: Input edge features (B x V x V x H)
        ...
        Ce = self.C(edge_attr)  # B x V x V x H

        # Update edge features and compute edge gates
        edge_attr = Ah.unsqueeze(1) + Bh.unsqueeze(2) + Ce  # B x V x V x H
        gates = torch.sigmoid(edge_attr)  # B x V x V x H
        ...
        edge_attr = self.norm_e(
            edge_attr.view(batch_size * num_nodes * num_nodes, hidden_dim)
        ).view(batch_size, num_nodes, num_nodes, hidden_dim) if self.norm_e else edge_attr
